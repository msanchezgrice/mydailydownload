...
async function sendEmail(to: string, subject: string, html: string, text: string, unsubscribeUrl: string): Promise<boolean> {
  ...
  try {
    ...
    if (!resp.ok) { console.error("Resend error:", resp.status); return false; }
    return true;
  } catch (e) { console.error("Resend send failed:", e); return false; }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  if (!token || !supabaseUrl || !serviceRoleKey) {
    res.status(400).send(page("Invalid link", `<h1 style="font-size:22px;">This confirmation link is invalid.</h1>`));
    return;
  }
  ...
  if (!sub) {
    res.status(404).send(page("Link not found", `<h1 style="font-size:22px;">This link has expired or was already used.</h1>`));
    return;
  }

  if (!sub.confirmed_at) {
    await supabase.from("subscribers").update({ confirmed_at: new Date().toISOString(), is_active: true }).eq("confirm_token", token);
    const unsubUrl = `${SITE}/api/unsubscribe?token=${sub.unsubscribe_token}`;
    const { subject, html, text } = welcomeEmail(humanizeCareer(sub.career_id), unsubUrl);
    await sendEmail(sub.email, subject, html, text, unsubUrl);
  }

  res.status(200).send(page("You're confirmed",
