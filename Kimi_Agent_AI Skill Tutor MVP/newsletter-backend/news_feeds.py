"""Curated list of real, free AI/tech RSS feeds (Lane A of the news engine).

Every feed here is a real, publicly-published RSS/Atom feed. No API keys are
required to read any of them. Each entry carries a human-readable `source`
name (used for citation in the briefing) and a coarse `tier` used only for
light prioritization when ranking.

If a feed URL ever 404s or stops publishing, the engine logs it and skips it
(per-feed try/except in news_engine.fetch_all) — a dead feed never breaks a run.
"""
